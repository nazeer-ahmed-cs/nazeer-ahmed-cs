<div align="center">
  <img src="https://raw.githubusercontent.com/nazeer-ahmed-cs/nazeer-ahmed-cs/main/banner.png" width="100%"/>
  <br/><br/>
  <h1>Nazeer Ahmed</h1>
  <h3>AI/ML Engineering Student · Python Developer · Open Source Contributor</h3>
  <p>
    Building machine learning models and production software that solve real-world problems.<br/>
    Focused on <strong>Computer Vision, Generative AI, LLMs, and Data Science</strong>.
  </p>
  <p>
    <a href="mailto:nazeerahmedkhaskhelin51@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white"/></a>
    <a href="https://www.linkedin.com/in/nazeer-ahmed-971382327/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white"/></a>
    <a href="https://nazeer-ahmed-cs.github.io"><img src="https://img.shields.io/badge/Portfolio-000000?style=flat-square&logo=githubpages&logoColor=white"/></a>
    <a href="https://github.com/nazeer-ahmed-cs"><img src="https://komarev.com/ghpvc/?username=nazeer-ahmed-cs&color=blue&style=flat-square"/></a>
  </p>
</div>

---

## About Me

- Studying **Computer Science** at Sukkur IBA University, specializing in Data Science & ML
- Building projects in **Computer Vision, NLP, Full-Stack, and LLM-powered applications**
- Experienced with **data cleaning, feature engineering, model deployment, REST APIs, and prompt engineering**
- Open to collaborating on **ML/DL and open-source projects**
- Ask me about **Python, Machine Learning, Computer Vision, LLMs, SQL, Data Structures**

---

## Tech Stack

**Languages:** Python · Java · C++ · SQL · JavaScript · HTML/CSS

**ML/DL:** TensorFlow · PyTorch · Scikit-learn · YOLOv8 · OpenCV · Pandas · NumPy

**Generative AI:** LangChain · Google Gemini · Prompt Engineering · LLM APIs

**Backend & DevOps:** Streamlit · FastAPI · Docker · Git · CI/CD · MongoDB · CustomTkinter

**Tools:** Power BI · Excel · Linux · VS Code · Jupyter · Postman

---

## Featured Projects

### 🚗 AI Vehicle Speed Detection System
YOLOv8 + custom IOU tracker for vehicle detection, tracking, and speed estimation with Streamlit web interface.
`Python` `YOLOv8` `OpenCV` `Streamlit` `Computer Vision`

### 🎬 CineVault Movie Management System
Full-featured desktop app with role-based authentication, movie CRUD, MongoDB, and PDF/Excel reporting.
`Python` `CustomTkinter` `MongoDB` `bcrypt` `OOP`

### 🤖 Smart Resume Analyzer
AI-powered resume analyzer using LangChain + Google Gemini. Upload a resume, paste a job description, get match score, skill gaps, and cover letter.
`Python` `LangChain` `Gemini` `Streamlit` `NLP` `LLM`

### 📵 Reel Break Enforcer
Windows productivity tool that monitors screen time and enforces break reminders with fullscreen overlays.
`Python` `tkinter` `Windows API` `ctypes`

### 🎥 MovieRecX Recommendation System
Movie recommendation engine using Hash Tables, Graphs, and Max-Heaps for fast search and similarity matching.
`Java` `Data Structures` `Algorithms`

---

## GitHub Stats

<p align="center">
  <img src="https://github-stats-extended.vercel.app/api?username=nazeer-ahmed-cs&show_icons=true&theme=tokyonight&count_private=true" width="48%"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=nazeer-ahmed-cs&theme=tokyonight" width="48%"/>
</p>
<p align="center">
  <img src="https://github-stats-extended.vercel.app/api/top-langs/?username=nazeer-ahmed-cs&layout=compact&theme=tokyonight" width="40%"/>
</p>

---

## Let's Connect

- **LinkedIn:** [nazeer-ahmed-971382327](https://www.linkedin.com/in/nazeer-ahmed-971382327/)
- **Email:** [nazeerahmedkhaskhelin51@gmail.com](mailto:nazeerahmedkhaskhelin51@gmail.com)
- **Portfolio:** [nazeer-ahmed-cs.github.io](https://nazeer-ahmed-cs.github.io)
- **Location:** Sukkur, Pakistan
